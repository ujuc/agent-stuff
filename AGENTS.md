...
├── amp/
│   ├── AGENTS.md            # Amp-specific global guidance; imports shared rules
│   └── settings.json        # Amp CLI user settings
...
├── rules/
│   ├── AGENTS.md            # Canonical cross-agent guidance (Claude/Amp imports + Codex symlink)
...
| `rules/AGENTS.md` | Canonical cross-agent guidance. Imported by `claude/CLAUDE.md` and `amp/AGENTS.md` (`@~/` paths); symlinked as `~/.codex/AGENTS.md`. Self-contained, < 8 KB. |
| `amp/AGENTS.md` | Global Amp-specific guidance, deployed as `~/.config/amp/AGENTS.md`. |
| `amp/settings.json` | Global Amp CLI settings, deployed as `~/.config/amp/settings.json`. |
