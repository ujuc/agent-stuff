...
description: |
  ...
  2~3분에 처리하고, --strict 모드는 5인 파이프라인(탐지→윤문→의미감사+자연성리뷰)
...
Fast 모드(디폴트, 단일 호출)와 Strict 모드(5인 파이프라인) 듀얼 트랙.
...
2. 사용자가 `--strict` 명시 또는 "정밀 모드", "5인 파이프라인" → **strict**
...
4. 등급 B 이하면 "정밀 검증이 필요하면 `--strict`로 5인 파이프라인 실행 가능" 안내
